  ...
  it("Quitar créditos a un game sin bot", () => {
    ...
    loginCustomer(testData)
      ...
      .then((withdrawResponse) => {
        ...
        expect(withdrawResponse.body.data.clientGame.state).to.eq("Processing")
        expect(withdrawResponse.body.data.clientGame.status).to.eq("Bot")
        expect(withdrawResponse.body.data.clientGame.gameCompanyId.removeCreditStatus).to.eq(true)

        cy.log(`Withdraw bot enviado para ${withdrawResponse.body.data.clientGame.gameMobileId}`)
      })
  })
})